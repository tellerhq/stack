/**
 * The tcp-service is similar to the `web-service` module, but
 * it provides a __public__ TCP ELB instead of HTTP(S).
 *
 * Usage:
 *
 *      module "auth_service" {
 *        source    = "github.com/tellerhq/stack//tcp-service"
 *        name      = "auth-service"
 *        image     = "auth-service"
 *        cluster   = "default"
 *      }
...
variable "protocol" {
  description = "ELB listener protocol"
  default     = "TCP"
}
